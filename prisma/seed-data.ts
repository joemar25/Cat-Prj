...
import { FormType, DocumentStatus, AttendantType, PrismaClient } from '@prisma/client'
...
const generatePhLocation = () => ({
    ...
    province: faker.helpers.arrayElement([
        ...
        'Pampanga', 'Bulacan', 'Batangas', 'Iloilo'
    ])
})
...
const generateBaseRegistryForm = (formType: FormType) => {
    ...
    return {
        formNumber: formType === FormType.MARRIAGE ? '97' :
            formType === FormType.BIRTH ? '102' : '103',
        formType,
...
export const generateMarriageCertificate = () => {
    ...
    return {
        ...
        marriageCertificateForm: {
            ...
            witnesses: Array(2).fill(null).map(() => ({
                name: faker.person.fullName(),
    ...
    return {
        ...
        deathCertificateForm: {
            ...
            causesOfDeath: {
                ...
                otherSignificant: faker.helpers.maybe(() => faker.helpers.arrayElement([
...
export const generateBulkData = async (prisma: PrismaClient, count = 1000): Promise<void> => {
    const records = {
        marriages: Math.floor(count * 0.4),  // 40% marriages
        births: Math.floor(count * 0.4),     // 40% births
        deaths: Math.floor(count * 0.2)      // 20% deaths
    }
...
export const generateAdditionalData = async (prisma: PrismaClient, userIds: string[]) => {
    ...
    const documents = await Promise.all(
        Array(10).fill(null).map(() => prisma.document.create({
            data: {
    ...
    const queueData = Array(queueCount).fill(null).map(() => ({
    ...
    const notificationData = Array(notificationCount).fill(null).map(() => ({
        userId: faker.helpers.arrayElement(userIds),
    ...
    const feedbackData = Array(feedbackCount).fill(null).map(() => ({
    ...
    for (let i = 0; i < workflowCount; i++) {
        await prisma.workflow.create({
            data: {
                ...
                steps: {
                    create: Array(4).fill(null).map((_, index) => ({
                        name: faker.helpers.arrayElement([
                            'Document Review',
                            'Data Verification',
                            'Quality Check',
